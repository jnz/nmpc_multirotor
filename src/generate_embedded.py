...
TARGETS = {
    ...
    "stm32f4": dict(
        description    = "STM32F4  (Cortex-M4, FPv4-SP)",
        blasfeo_target = "GENERIC",
        hpipm_target   = "EMBEDDED",
        cc             = "arm-none-eabi-gcc",
        ar             = "arm-none-eabi-ar",
        ...
        c_defs         = "-DBLASFEO_TARGET_GENERIC"
                         " -DHPIPM_TARGET_EMBEDDED -DACADOS_SILENT",
        notes          = "Cortex-M4 has single-precision FPU only. Verify timing budget.",
    ),
    "rpi4": dict(
    ...
    "esp32s3": dict(
        description    = "ESP32-S3  (Xtensa LX7, no HW BLAS)",
        blasfeo_target = "GENERIC",
        hpipm_target   = "EMBEDDED",
        cc             = "xtensa-esp32s3-elf-gcc",
        ar             = "xtensa-esp32s3-elf-ar",
        c_flags        = "-O2 -ffast-math -mlongcalls",
        c_defs         = "-DBLASFEO_TARGET_GENERIC"
                         " -DHPIPM_TARGET_EMBEDDED -DACADOS_SILENT",
        notes          = "GENERIC BLASFEO is slower - verify timing budget."
...
CFLAGS      += -I. -Ic_generated_code -Ilibs/include -Ilibs/include/blasfeo/include -Ilibs/include/hpipm/include
...
clean:
\trm -rf $(BUILD_DIR) $(LIBS_DIR) $(OBJ_DIR)
...
{{
    ...
    x0[NMPC_IDX_Q0] = 1.0;   /* unit quaternion: level, no rotation */
...
{{
    ...
    sp.pos_ned[2] = -5.0;   /* Down  [m]  (negative = above ground) */
...
def generate(vehicle_id: int, target_id: str, outdir: str):
    ...
    if is_embedded:
        ...
        toolchain_flag_str = ("-DCMAKE_C_COMPILER=\"%s\" \\\\n\t    -DCMAKE_AR=\"%s\" " % (target["cc"], target["ar"]))
